from . import Model_Utils, layers, layerspp, dense_layer
...
class NCSNpp(nn.Module):
    def __init__(self, config):
        ...
        channels = config.numG_channels
        out_channel = config.num_channels
        ...
        if progressive != 'output_skip':
            modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                        num_channels=in_ch, eps=1e-6))
            modules.append(conv3x3(in_ch, out_channel, init_scale=init_scale))
    ...
    def forward(self, x, time_cond, z, x_cond, dcp_T):
        ...
        m_idx = 0
        x = torch.cat([x,x_cond, dcp_T], dim=1)
