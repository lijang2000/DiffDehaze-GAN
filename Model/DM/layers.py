...
def variance_scaling(scale, mode, distribution, in_axis=1, out_axis=0, dtype=torch.float32, device='cpu'):
    """
    :param scale:缩放因子
    :param mode:模式，决定了如何计算方差的缩放方式，可以是 "fan_in", "fan_out", 或 "fan_avg"
    :param distribution:初始化的分布类型，可以是 "normal" 或 "uniform"
    :param in_axis, out_axis:指定输入和输出轴的索引，默认为 1 和 0
    :param dtype:数据类型，默认为 torch.float32
